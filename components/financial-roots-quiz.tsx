...
export function FinancialRootsQuiz() {
  ...
  // All quiz questions - simplified structure
  const baseQuestions = [
  ...
  // Build the current question list based on state
  const buildQuestionList = () => {
    let questions = [...baseQuestions, businessCheckQuestion]

    if (hasBusiness === true) {
      questions = [...questions, ...businessQuestions]
    }

    return questions
  }

  const currentQuestions = buildQuestionList()
  const currentQuestionData = currentQuestions[currentQuestion]

  const handleAnswer = (answerValue: any) => {
    if (!currentQuestionData) return

    // Handle business check question
  ...
  const calculateResults = () => {
    ...
    const sectionResults: SectionResult[] = sections.map((section) => {
      const sectionAnswers = answers.filter((a) => {
        const question = currentQuestions.find((q) => q.id === a.questionId)
        return question?.section === section
      })

      const totalScore = sectionAnswers.reduce((sum, a) => sum + a.score, 0)
      const maxScore = sectionAnswers.length * 2
  ...
  if (!currentQuestionData) {
    return (
      <Card className="border-2 border-[#A8C686] shadow-xl">
        <CardContent className="p-8 text-center">
          <h3 className="text-xl font-bold text-[#1F4E45] mb-4">Quiz Complete!</h3>
          <p className="text-[#1F4E45]/70 mb-4">Thank you for completing the Financial Roots Assessment.</p>
          <Button onClick={() => setShowResult(true)} className="bg-[#1F4E45] hover:bg-[#1F4E45]/90 text-white">
            View Results
          </Button>
        </CardContent>
      </Card>
