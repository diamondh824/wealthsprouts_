...
export function FinancialRootsQuiz() {
  ...
  // Define all questions as a constant to prevent re-creation
  ...
  // Get the questions to show based on current state - memoized for performance
  const questionsToShow = useMemo((): QuizQuestion[] => {
    try {
      ...
      return allQuestions.slice(0, 18) // Fallback to base questions only
    }
  }, [allQuestions, hasBusiness])

  // Get current question data safely
  const currentQuestionData = useMemo(() => {
    if (currentQuestion >= 0 && currentQuestion < questionsToShow.length) {
      return questionsToShow[currentQuestion]
    }
    return null
  }, [currentQuestion, questionsToShow])

  // Handle answer submission with better error handling
  const handleAnswer = useCallback(
    (answerValue: any) => {
      try {
        ...
        if (currentQuestionData.type === "yes-no") {
          ...
          score = Number(answerValue) || 0
          if (score <= 2 && currentQuestionData.tag) {
            tags.push(currentQuestionData.tag)
          }
        }
    ...
    [currentQuestionData, currentQuestion, questionsToShow.length],
  )

  // Calculate results with error handling
  const calculateResults = useCallback((): FinalResult => {
    try {
      const sections = [
        "Budgeting & Spending",
        "Credit Health",
      ...
      const sectionResults: SectionResult[] = sections.map((section) => {
        ...
        const totalScore = sectionAnswers.reduce((sum, a) => sum + (a.score || 0), 0)
      ...
      const overallScore =
        sectionResults.length > 0 ? sectionResults.reduce((sum, s) => sum + s.percentage, 0) / sectionResults.length : 0
      ...
      return {
        ...
        sectionResults: [],
        tags: [],
        recommendations: ["Please retake the quiz or contact support for assistance."],
      }
    }
  }, [answers, allQuestions, hasBusiness])

  // Render answer options with error handling
  const renderAnswerOptions = useCallback(() => {
    ...
    try {
      ...
      console.error("Error rendering answer options:", error)
      return <div className="text-center p-4 text-red-600">Error loading answer options. Please refresh the page.</div>
    }
  }, [currentQuestionData, handleAnswer])
        ...
        <Card className="border-2 border-[#A8C686] shadow-xl">
          <CardContent className="p-8 text-center">
            ...
            <Button
              ...
              onClick={() => handleExternalLink("https://www.oncehub.com/wealthsprouts")}
            >
  ...
  // Show error state if no current question
  if (!currentQuestionData) {
    return (
      <Card className="border-2 border-red-300 shadow-xl">
        <CardContent className="p-8 text-center">
          <h3 className="text-xl font-bold text-red-600 mb-4">Quiz Loading Error</h3>
          <p className="text-gray-600 mb-4">
            We're having trouble loading the quiz. Please refresh the page and try again.
          </p>
          <Button onClick={() => window.location.reload()} className="bg-[#1F4E45] hover:bg-[#1F4E45]/90 text-white">
            Refresh Page
          </Button>
        </CardContent>
      </Card>
